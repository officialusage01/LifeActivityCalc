...
function App() {
  ...
  const [viewMode, setViewMode] = useState<'standard' | 'story'>('standard');
  const [currentStoryStep, setCurrentStoryStep] = useState(0);
  
  const resultsRef = useRef<HTMLDivElement>(null);
  const { isMobile } = useDeviceDetect();

  // Set initial view mode based on device detection
  useEffect(() => {
    setViewMode(isMobile ? 'story' : 'standard');
  }, [isMobile]);
  ...
  const calculateHours = () => {
    ...
    // If in standard view, scroll to results
    if (viewMode === 'standard') {
      setTimeout(() => {
        resultsRef.current?.scrollIntoView({ behavior: 'smooth' });
      }, 100);
    } else {
      // If in story view, go to results step
      setCurrentStoryStep(2);
    }
  };

  const handleDownloadPDF = () => {
    if (!results || !age) return;
  ...
  const BirthDateStep = () => (
    <div className="h-full flex flex-col justify-center p-6 bg-gradient-to-br from-blue-50 to-indigo-100">
      ...
      <p className="text-center text-gray-600">Swipe left to continue</p>
    </div>
  );

  const ActivitiesStep = () => (
    <div className="h-full flex flex-col justify-center p-6 bg-gradient-to-br from-blue-50 to-indigo-100 overflow-y-auto">
      <div className="bg-white rounded-xl shadow-lg p-6 mb-8 max-w-md mx-auto">
        ...
        <div className={`flex items-center justify-between mb-6 p-3 rounded-lg ${
          ...
          <div className="flex items-center">
            ...
            <p>
              Total: <span className="font-bold">{totalDailyHours.toFixed(1)}</span> / 24
            </p>
          </div>
        ...
        <div className="mt-8">
          <button
            ...
            disabled={exceeds24Hours || !birthDate}
  ...
  // Toggle between standard and story view
  const toggleViewMode = () => {
    setViewMode(prev => prev === 'standard' ? 'story' : 'standard');
  };

  // Listen for window resize to update view mode based on screen size
  useEffect(() => {
    const handleResize = () => {
      // Only auto-switch if user hasn't manually toggled
      if ((isMobile && viewMode === 'standard') || (!isMobile && viewMode === 'story')) {
        setViewMode(isMobile ? 'story' : 'standard');
      }
    };
  ...
  return (
    <div className={`min-h-screen ${viewMode === 'standard' ? 'bg-gradient-to-br from-blue-50 to-indigo-100' : ''}`}>
      {viewMode === 'standard' ? (
        <div className="container mx-auto px-4 py-8">
          <header className="text-center mb-10">
            <h1 className="text-4xl font-bold text-indigo-800 mb-2">Life Activity Calculator</h1>
            <p className="text-gray-600 mb-4">Discover how you've spent your time throughout your life</p>
            <button 
        ...
        <div className="h-screen flex flex-col">
          <header className="bg-white shadow-sm p-4 flex justify-between items-center">
            <h1 className="text-xl font-bold text-indigo-800">Life Activity Story</h1>
            <button 
              onClick={toggleViewMode}
              className="text-indigo-600 hover:text-indigo-800 text-sm font-medium"
            >
