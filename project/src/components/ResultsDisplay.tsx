...
const ResultsDisplay: React.FC<ResultsDisplayProps> = ({ results, age }) => {
  const [activeTab, setActiveTab] = useState<'summary' | 'chart' | 'percentage'>('summary');
  ...
  useEffect(() => {
    ...
    sortedActivities.forEach(([key, value], index) => {
      ...
      ctx.font = '12px Arial';
      ctx.textAlign = 'center';
      ctx.fillText(activityLabels[key], x + barWidth / 2, canvasHeight - 10);
    });
  }, [results, activeTab]);
  ...
  const calculateFacts = () => {
    ...
    const facts = [];
    const totalHours = Object.values(results).reduce((sum, val) => sum + val, 0);
    ...
    const highestActivity = Object.entries(percentages)
      .sort(([, a], [, b]) => b - a)[0];
    
    if (highestActivity) {
      facts.push(`Your most time-consuming activity is ${activityLabels[highestActivity[0]]}, taking up ${highestActivity[1].toFixed(1)}% of your tracked time.`);
    }
  ...
  const downloadCSV = () => {
    ...
    Object.entries(results).forEach(([key, value]) => {
      csvContent += `${activityLabels[key]},${value.toFixed(2)},${percentages[key].toFixed(2)}%\n`;
    });
      ...
      <div className="flex mb-6 bg-gray-100 p-1 rounded-lg">
        <button
          className={`flex-1 py-2 px-4 rounded-md transition-all ${
            activeTab === 'summary' ? 'bg-white shadow-md text-indigo-700 font-medium' : 'text-gray-600 hover:bg-gray-200'
          }`}
        ...
        <button
          className={`flex-1 py-2 px-4 rounded-md transition-all ${
            activeTab === 'chart' ? 'bg-white shadow-md text-indigo-700 font-medium' : 'text-gray-600 hover:bg-gray-200'
          }`}
        ...
        <button
          className={`flex-1 py-2 px-4 rounded-md transition-all ${
            activeTab === 'percentage' ? 'bg-white shadow-md text-indigo-700 font-medium' : 'text-gray-600 hover:bg-gray-200'
          }`}
              ...
              .map(([key, value]) => (
                <div key={key} className="bg-gray-50 p-4 rounded-lg border border-gray-200">
                  <div className="flex items-center mb-2">
                    ...
                    <h3 className="font-medium text-gray-800">{activityLabels[key]}</h3>
                  </div>
                  <p className="text-2xl font-bold text-indigo-700">{formatHours(value)}</p>
                </div>
              ))}
          </div>
          
          <div className="mt-8 bg-indigo-50 p-4 rounded-lg">
            <h3 className="font-semibold text-indigo-800 mb-3">Interesting Facts</h3>
      ...
      {activeTab === 'percentage' && (
        <div>
          <p className="text-gray-600 mb-4">
            Here's how your time is distributed as a percentage of your total tracked time:
          </p>
              ...
              .map(([key, percentage]) => (
                <div key={key} className="mb-2">
                  <div className="flex justify-between mb-1">
                    ...
                    <span className="text-sm font-medium text-indigo-700">{percentage.toFixed(1)}%</span>
                  </div>
          ...
          <div className="mt-8 p-4 bg-gray-50 rounded-lg">
            ...
            <p className="text-gray-600">
              {Object.values(results).reduce((sum, val) => sum + val, 0).toFixed(0)} total hours tracked, 
              which is approximately {(Object.values(results).reduce((sum, val) => sum + val, 0) / 24 / 365).toFixed(1)} years 
              of your {age.toFixed(1)} years of life.
            </p>
          </div>
        </div>
      )}
